...
public class MapFunctions
{
	...
	/// <summary>
	/// Creates a cave using multiple layers of perlin noise for more detailed generation
	/// </summary>
    ...
    {
        // Use different scales for each noise layer
        float primaryScale = modifier;
        float secondaryScale = modifier * 2.5f;
        
        // Use offset derived from seed for the second layer of noise
        float secondLayerOffset = seed * 573.26f;
        ...
        for (int x = 0; x < map.GetUpperBound(0); x++)
        {
            for (int y = 0; y < map.GetUpperBound(1); y++)
            {
                ...
                else
                {
                    ...
                    float primaryNoise = Mathf.PerlinNoise(x * primaryScale + seed, y * primaryScale + seed);
                    
                    // Generate secondary noise layer with different scale and offset
                    float secondaryNoise = Mathf.PerlinNoise(x * secondaryScale + secondLayerOffset, y * secondaryScale + secondLayerOffset);
                    
                    // Combine both noise layers with weights
                    // 70% primary layer, 30% secondary layer
                    float combinedNoise = primaryNoise * 0.7f + secondaryNoise * 0.3f;
                    
                    // Round to binary value (0 or 1)
                    int newPoint = Mathf.RoundToInt(combinedNoise);
                    map[x, y] = newPoint;
                }
            }
        }
